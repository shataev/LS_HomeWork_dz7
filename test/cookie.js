...
describe('ДЗ 7.2 - Cookie editor', () => {
    ...
    describe('Интеграционное тестирование', () => {
        ...
        it('cookie должны добавляться при нажатии на "добавить"', () => {
            ...
            cookies = getCookies();
            assert(cookies.hasOwnProperty(cookieName), 'cookie не добавлена в барузер');
            assert.equal(cookies[cookieName], cookieValue, 'cookie не добавлена в барузер');
        ...
        it('если при добавлении указано имя существующей cookie, то в таблице не должно быть дублей', () => {
            ...
            cookies = getCookies();
            assert(cookies.hasOwnProperty(cookieName), 'cookie не добавлена в барузер');
            assert.equal(cookies[cookieName], cookieValue, 'не изменено значение cookie');
        ...
        describe('Фильтрация', () => {
            ...
            it('добавлять cookie в таблицу, только если значение cookie соответствует фильтру', () => {
                ...
                cookies = getCookies();
                assert(cookies.hasOwnProperty(cookieName), 'должна быть добавлена в браузер');
                assert.equal(cookies[cookieName], cookieValue, 'должна быть добавлена в браузер');
            ...
            it('удалить cookie из табилицы, если ее значение перестало соответствовать фильтр', () => {
                ...
                cookies = getCookies();
                assert(cookies.hasOwnProperty(cookieName), 'должна оставться в браузере');
                assert.equal(cookies[cookieName], cookieValue, 'значение в браузере должно измениться');
